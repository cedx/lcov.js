...
export class Report {
	...
	static fromJson(map) {
		return new this(
			...
			Array.isArray(map.sourceFiles) ? map.sourceFiles.map(item => SourceFile.fromJson(item)) : []
		);
	}
	...
	static fromString(coverage) {
		...
		for (let line of coverage.split(/\r?\n/g)) {
			...
			switch (token) {
				...
				case Token.testName: if (!report.testName.length) report.testName = data[0]; break;
