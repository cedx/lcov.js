...
export class BranchData {
	...
	static fromJson(map) {
		return new this({
			blockNumber: Number.isInteger(map.blockNumber) ? map.blockNumber : 0,
			branchNumber: Number.isInteger(map.branchNumber) ? map.branchNumber : 0,
			lineNumber: Number.isInteger(map.lineNumber) ? map.lineNumber : 0,
			taken: Number.isInteger(map.taken) ? map.taken : 0
		});
	}
